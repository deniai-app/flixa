...
export function CustomDropdown({ options, value, onChange, iconOnly }: CustomDropdownProps) {
  ...
  useEffect(() => {
    ...
    const baseBottom = window.innerHeight - rect.bottom + 4;
        ...
        {options.map((option) => (
          <button
            ...
            className={`w-full px-3 py-2 cursor-pointer transition-colors border-b border-menu-separator last:border-b-0 text-left ${
              option.value === value ? "bg-menu-selected-bg" : "hover:bg-menu-selected-bg"
            }`}
            onClick={() => {
          ...
          >
            <div className="flex items-center gap-1.5">
              ...
              <span
                className={`text-xs font-medium ${option.value === value ? "text-menu-selected-foreground" : "text-menu-foreground"}`}
              >
                {option.label}
              </span>
              {option.value === value && (
